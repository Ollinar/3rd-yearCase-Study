<div class="grid gap-6 h-auto md:grid-cols-2 xl:grid-cols-4  hover:cursor-pointer" id="tabCard" hx-swap-oob="innerHTML">
    ...
    <a href="/?type=unclaimed" hx-get="/?type=unclaimed" hx-param="*" hx-target="#cardCont" hx-push-url="true">
        <div class="flex items-center h-32 border p-4 <?php if (isset($_GET['type'])) {
            $type = $_GET['type'];
            echo $type === 'unclaimed' ? 'bg-blue-100' : 'bg-white';
        } ?> rounded-lg shadow-xl dark:bg-gray-800">
            <div class="p-3 mr-4 text-blue-500 bg-blue-100 rounded-full dark:text-blue-100 dark:bg-blue-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6">
